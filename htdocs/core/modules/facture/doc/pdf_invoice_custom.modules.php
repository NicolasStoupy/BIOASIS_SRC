 ...
 */
class pdf_invoice_custom extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "invoice_custom";
		$this->description = 'Facture custom pour Bioasis';
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				//PAGE FOR TERMS OF SALES
				$pdf->SetFont('', 'B', 26);
				$pdf->MultiCell(200, 1, 'Conditions générales de vente', 0, 'C', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '1. Champ d\'application', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 5, 'Les présentes conditions générales de vente s\'appliquent à toutes les ventes de produits réalisées par Bioasis, 
				société SRL, dont le siège social est situé à Namur, enregistrée sous le numéro BE1002685634.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '2. Commandes', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 5, 'Les commandes de produits peuvent être passées par téléphone, par e-mail ou via notre site web.
				Toute commande passée constitue une offre d\'achat de la part du client, et est soumise à l\'acceptation du Vendeur.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '3. Prix', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 5, 'Les prix des produits sont indiqués en euros et sont ceux en vigueur au moment de la commande, hors taxes et frais de livraison.
				Le Vendeur se réserve le droit de modifier ses prix à tout moment, mais les produits seront facturés sur la base des tarifs en vigueur au moment de la passation de la commande.' , 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '4. Livraison', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 5, 'Les délais de livraison sont donnés à titre indicatif et peuvent varier en fonction des circonstances.
				Les frais de livraison sont à la charge du client et seront indiqués au moment de la commande.
				Le risque de perte ou d\'endommagement des produits est transféré au client dès leur remise par le Vendeur au transporteur.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '5. Paiement', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 3, 'Sauf disposition contraire, les factures sont payables à réception.
				En cas de retard de paiement, des pénalités de retard seront appliquées conformément à la législation en vigueur.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '6. Réclamations', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 3, 'Les réclamations concernant les produits livrés doivent être formulées par écrit dans un délai de 7 jours à compter de la réception de la commande.
				Le client doit fournir toute preuve raisonnable à l\'appui de sa réclamation.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '7. Droit de rétractation', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 3, 'Conformément à la législation en vigueur, le client dispose d\'un délai de 14 jours à compter de la réception des produits
				 pour exercer son droit de rétractation, sans avoir à justifier de motifs ni à payer de pénalités.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '8. Responsabilité', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 3, 'La responsabilité du Vendeur ne saurait être engagée en cas de dommages indirects résultant de l\'utilisation des produits vendus.', 0, 'L', 0);
				$pdf->SetFont('', 'B', $default_font_size + 4);
				$pdf->MultiCell(200, 1, '9. Loi applicable et juridiction compétente', 0, 'L', 0);
				$pdf->SetFont('', '', $default_font_size);
				$pdf->MultiCell(200, 3, 'Les présentes conditions générales de vente sont régies par le droit belge.
				Tout litige relatif à leur interprétation ou à leur exécution sera de la compétence exclusive des tribunaux de Namur.', 0, 'L', 0);
